...
#ifdef OPLUS_ARCH_EXTENDS
static const struct file_operations oplus_debug_dbmdx_reboot_fops = {
	.owner   = THIS_MODULE,
	.write   = dbmdx_dev_reboot_store,
};

static struct miscdevice oplus_debug_dbmdx_reboot_device = {
	.minor = MISC_DYNAMIC_MINOR,
	.name = "dbmdx_reboot_dev",
	.fops = &oplus_debug_dbmdx_reboot_fops,
};

static const struct file_operations oplus_debug_dbmdx_va_load_amodel_fops = {
	.owner   = THIS_MODULE,
	.write   = dbmdx_dev_va_acoustic_model_store,
};

static struct miscdevice oplus_debug_dbmdx_va_load_amodel_device = {
	.minor = MISC_DYNAMIC_MINOR,
	.name = "dbmdx_valoadamodel_dev",
	.fops = &oplus_debug_dbmdx_va_load_amodel_fops,
};
#endif /* OPLUS_ARCH_EXTENDS */
...
#ifdef OPLUS_ARCH_EXTENDS
	ret = misc_register(&oplus_debug_dbmdx_reboot_device);
	if (ret) {
		pr_err("%s: dbmdx reboot misc_register Fail:%d\n", __func__, ret);
		goto out_err_destroy_workqueue;
	}

	ret = misc_register(&oplus_debug_dbmdx_va_load_amodel_device);
	if (ret) {
