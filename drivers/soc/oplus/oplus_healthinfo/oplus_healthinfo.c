...
struct sched_stat_para oplus_sched_para[OHM_SCHED_TOTAL];
...
{
    struct sched_stat_para *sched_stat = &oplus_sched_para[sched_type];
    ...
    if (task_is_fg(task)) {
        ...
        if (unlikely(delta_ms >= sched_stat->high_thresh_ms)){
             if (oplus_sched_para[sched_type].logon  && __ratelimit(&ratelimit)) {
                        ohm_debug_deferred("[%s / %s] high_cnt, delay = %llu ms\n",
                                sched_list[sched_type], "fg", delta_ms);
                }
             if (oplus_sched_para[sched_type].trig)
...
void ohm_para_update(void)
{
        ...
        for (i = 0 ;i < OHM_SCHED_TOTAL ;i++ ) {
                ...
                oplus_sched_para[i].low_thresh_ms = ohm_thresh_para[i].l_ms;
                oplus_sched_para[i].high_thresh_ms = ohm_thresh_para[i].h_ms;
        }
...
void ohm_trig_init(void)
{
        ...
        for (i = 0 ;i < OHM_SCHED_TOTAL ;i++ ) {
                oplus_sched_para[i].trig = (ohm_trig_list & BIT(i)) ? true : false;
		}
        return;
}
...
void ohm_logon_init(void)
{
        ...
        for (i = 0 ;i < OHM_SCHED_TOTAL ;i++ ) {
                oplus_sched_para[i].logon = (ohm_logon_list & BIT(i)) ? true : false;
		}
        return;
}
...
void ohm_ctrl_init(void)
{
        ...
        for (i = 0 ;i < OHM_SCHED_TOTAL ;i++ ) {
                oplus_sched_para[i].ctrl = (ohm_ctrl_list & BIT(i)) ? true : false;
		}
        return;
}
...
void ohm_para_init(void)
{
        int i;
        for (i = 0 ;i < OHM_SCHED_TOTAL ;i++ ) {
               _ohm_para_init(&oplus_sched_para[i]);
               oplus_sched_para[i].low_thresh_ms = 100;
               oplus_sched_para[i].high_thresh_ms = 500;
        }
        oplus_sched_para[OHM_SCHED_EMMCIO].low_thresh_ms = 100;
        oplus_sched_para[OHM_SCHED_EMMCIO].high_thresh_ms = 200;
        ohm_ctrl_init();
        ohm_logon_init();
        ohm_trig_init();
...
static ssize_t sched_latency_read(struct file *filp, char __user *buff, size_t count, loff_t *off)
{
    ...
    int len = 0;
    struct sched_stat_para *sched_stat = &oplus_sched_para[OHM_SCHED_SCHEDLATENCY];
...
static ssize_t iowait_read(struct file *filp, char __user *buff, size_t count, loff_t *off)
{
    char page[BUFFER_SIZE_L] = {0};
    int len = 0;
    struct sched_stat_para *sched_stat = &oplus_sched_para[OHM_SCHED_IOWAIT];
...
static ssize_t fsync_wait_read(struct file *filp, char __user *buff, size_t count, loff_t *off)
{
    char page[BUFFER_SIZE_L] = {0};
    int len = 0;
    struct sched_stat_para *sched_stat = &oplus_sched_para[OHM_SCHED_FSYNC];
...
static ssize_t emmcio_read(struct file *filp, char __user *buff, size_t count, loff_t *off)
{
      int len = 0;
      char page[1024] = {0};
      struct sched_stat_para *sched_stat = &oplus_sched_para[OHM_SCHED_EMMCIO];
...
static ssize_t dstate_read(struct file *filp, char __user *buff, size_t count, loff_t *off)
{
        char page[BUFFER_SIZE_L] = {0};
        int len = 0;
        struct sched_stat_para *sched_stat = &oplus_sched_para[OHM_SCHED_DSTATE];
...
static int __init oplus_healthinfo_init(void)
{
	...
	struct proc_dir_entry *pentry;
        ...
        ohm_para_init();
        ohm_action_init();
	oplus_healthinfo =  proc_mkdir(HEALTHINFO_PROC_NODE, NULL);
	if(!oplus_healthinfo) {
		ohm_err("can't create oplus_healthinfo proc\n");
		goto ERROR_INIT_VERSION;
	}
/******  ctrl  *****/
	pentry = proc_create("para_update", S_IRUGO | S_IWUGO, oplus_healthinfo, &proc_para_fops);
        if(!pentry) {
...
/******  Stat  ******/
	pentry = proc_create("fsync_wait", S_IRUGO, oplus_healthinfo, &proc_fsync_wait_fops);
	if(!pentry) {
		ohm_err("create fsync_wait proc failed.\n");
		goto ERROR_INIT_VERSION;
	}

	pentry = proc_create("cpu_loading", S_IRUGO, oplus_healthinfo, &proc_cpu_load_fops);
	if(!pentry) {
		ohm_err("create cpu_loading proc failed.\n");
		goto ERROR_INIT_VERSION;
	}

	pentry = proc_create("iowait", S_IRUGO, oplus_healthinfo, &proc_iowait_fops);
	if(!pentry) {
		ohm_err("create iowait proc failed.\n");
		goto ERROR_INIT_VERSION;
	}

	pentry = proc_create("sched_latency", S_IRUGO, oplus_healthinfo, &proc_sched_latency_fops);
	if(!pentry) {
		ohm_err("create sched_latency proc failed.\n");
		goto ERROR_INIT_VERSION;
	}

	pentry = proc_create("emcdrv_iowait", S_IRUGO, oplus_healthinfo, &proc_emmcio_fops);
	if(!pentry) {
		ohm_err("create emmc_driver_io_wait proc failed.\n");
		goto ERROR_INIT_VERSION;
	}
	
	pentry = proc_create("dstate", S_IRUGO, oplus_healthinfo, &proc_dstate_fops);
	if(!pentry) {
		ohm_err("create dstate proc failed.\n");
		goto ERROR_INIT_VERSION;
	}

	pentry = proc_create("iowait_hung", S_IRUGO, oplus_healthinfo, &proc_iowait_hung_fops);
	if(!pentry) {
		ohm_err("create iowait_hung proc failed.\n");
		goto ERROR_INIT_VERSION;
	}
	
	pentry = proc_create("ion_wait", S_IRUGO, oplus_healthinfo, &proc_ion_wait_fops);
	if(!pentry) {
		ohm_err("create ion_wait proc failed.\n");
		goto ERROR_INIT_VERSION;
	}
#ifdef CONFIG_OPLUS_MEM_MONITOR
	pentry = proc_create("alloc_wait", S_IRUGO, oplus_healthinfo, &proc_alloc_wait_fops);
	if(!pentry) {
    ...
    pentry = proc_create("cpu_info", S_IRUGO, oplus_healthinfo, &proc_cpu_info_fops);
    if(!pentry) {
...
#ifdef OPLUS_FEATURE_UFSPLUS
#if defined(CONFIG_UFSFEATURE)
	pentry = proc_create("ufs_feature", S_IRUGO, oplus_healthinfo, &proc_ufsf_para_fops);
	if(!pentry) {
...
#ifdef OPLUS_FEATURE_SDCARD_INFO
	pentry = proc_create("sd_info", S_IRUGO, oplus_healthinfo, &proc_sdinfo_fops);
	if(!pentry) {
...
/****** thresh update ******/
    sched_thresh =  proc_mkdir("sched_thresh", oplus_healthinfo);
    if(!oplus_healthinfo) {
        ohm_err("can't create oplus_healthinfo proc\n");
        goto ERROR_INIT_VERSION;
    }
	...
	ret = create_reserved_area_enable_proc(oplus_healthinfo);
	if (ret)
		goto ERROR_INIT_VERSION;
#endif
        ohm_debug("Success \n");
...
MODULE_DESCRIPTION("OPLUS healthinfo monitor");
